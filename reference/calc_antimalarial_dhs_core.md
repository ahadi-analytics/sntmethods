...
- `dhs_antimalarial`: Proportion receiving any antimalarial

- `dhs_antimalarial_low`, `dhs_antimalarial_upp`: 95\\

- `dhs_n_febrile`: Number of febrile children (denominator)

- `dhs_n_antimalarial`: Number receiving any antimalarial
...
Auto-detects the available drug series at runtime: ml13\* variables
(ml13a through ml13g, ml13aa, etc.) are preferred. If absent, falls back
to the h37a-h series used in older DHS surveys (e.g. BFA 2021).
received_antimalarial = 1 if ANY detected drug variable equals 1. ACT
(ml13e / h37e) is a subset.

## See also

[`calc_act_dhs()`](https://ahadi-analytics.github.io/sntmethods/reference/calc_act_dhs.md)
for ACT-specific treatment (step 4),
