# ============================================================================
# Tests for calc_itn_dhs() — long-format ITN indicators
# ============================================================================
...
test_that("calc_itn_dhs returns named list with adm0", {
  mocks <- make_itn_mock()
  result <- calc_itn_dhs(dhs_hr = mocks$hr, dhs_pr = mocks$pr)
...
test_that("calc_itn_dhs validates inputs", {
  mocks <- make_itn_mock()

  expect_error(calc_itn_dhs(dhs_hr = "not a df", dhs_pr = mocks$pr))
  expect_error(calc_itn_dhs(dhs_hr = mocks$hr, dhs_pr = "not a df"))
  expect_error(calc_itn_dhs(dhs_hr = data.frame(), dhs_pr = mocks$pr))
  expect_error(calc_itn_dhs(dhs_hr = mocks$hr, dhs_pr = data.frame()))
})


# --- Test: itn_dictionary() -----------------------------------------------

test_that("itn_dictionary returns correct structure", {
  dict <- itn_dictionary()
